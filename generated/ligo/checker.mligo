...
let assert_checker_invariants (state: checker) : unit =
  ...
  List.iter
    (fun (burrow_address, burrow) ->
       match Big_map.find_opt burrow_address state.liquidation_auctions.burrow_slices with
       | None ->
    ...
    (Big_map.bindings state.burrows)
...
let view_current_liquidation_auction_details ((), state: unit * checker) : view_current_liquidation_auction_details_result =
  ...
  let current_bid, blocks, seconds = match auction.state with
    ...
    | Ascending (current_bid, bid_time_seconds, bid_level) ->
      (
        Some current_bid,
        Some (sub_nat_nat (add_nat_nat bid_level max_bid_interval_in_blocks) (Tezos.get_level ())),
        Some (sub_timestamp_timestamp (add_timestamp_int bid_time_seconds max_bid_interval_in_seconds) (Tezos.get_now ()))
      )
  in
  {
