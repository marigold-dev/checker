...
let assert_burrow_slices_invariants auctions burrow_id =
  let burrow_slices = match Big_map.find_opt burrow_id auctions.burrow_slices with
    | Some bs -> bs
...
let assert_liquidation_auction_invariants (auctions: liquidation_auctions) : unit =
  ...
  let roots = Big_map.bindings mem.mem
  ...
  let mem = auctions.avl_storage in
  let nodes = Big_map.bindings mem.mem
  ...
  let _ = List.map (
      ...
      (Big_map.bindings auctions.burrow_slices) in
  ...
  kit_max (kit_of_denomination (1n))
    (match auction.state with
     | Descending (start_value, start_time) ->
       ...
       let time_passed = sub_timestamp_timestamp (Tezos.get_now ()) start_time in
...
let liquidation_auction_place_bid (auction: current_liquidation_auction) (bid: bid) : (current_liquidation_auction * (bid option)) =
  ...
  then
    begin
      let updated_auction = { auction with state = Ascending (bid, (Tezos.get_now ()), (Tezos.get_level ())); } in
